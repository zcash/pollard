...
fn mds_array<F: FieldExt, S: Spec<F>>(matrix: Vec<S::State>) -> [[F; WIDTH]; WIDTH] {
    assert_eq!(matrix.len(), WIDTH);
    let mut arr = matrix.into_iter().map(|v| v.as_ref().try_into().unwrap());
    [
        arr.next().unwrap(),
        arr.next().unwrap(),
        arr.next().unwrap(),
...
impl<F: FieldExt> Chip for Pow5T3Chip<F> {
    type Field = F;
...
impl<F: FieldExt> PoseidonInstructions for Pow5T3Chip<F> {
...
impl<F: FieldExt> Pow5T3State<F> {
    ...
    fn round(
        ...
        round_fn: impl FnOnce(&mut Region<Pow5T3Chip<F>>) -> Result<(usize, [Option<F>; WIDTH]), Error>,
    ) -> Result<Self, Error> {
...
mod tests {
    ...
    impl Circuit<Fp> for MyCircuit {
        ...
        fn synthesize(
            ...
            let final_state = Pow5T3Chip::permute(&mut layouter, &initial_state)?;
            ...
            poseidon::permute::<_, OrchardNullifier>(
